...
function AdvertisementStudent({ advertisement }) {
  ...
  const getDetails = async () => {
    try {
      ...
      const response = await fetch(
        `${API_SERVER_URL}/api/profile/company/${advertisement.companyId}`,
        {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch(
      `${API_SERVER_URL}/api/enrollment/applications/${advertisement.advertisementId}`,
      {
        method: "POST",
        headers: {
  ...
  return (
    <div className="bg-white rounded-lg border p-4 mb-4">
      ...
      {showDetails && (
        <div className=" fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-10">
          <div className="bg-white p-6 rounded-lg w-2/3 max-h-[90vh] overflow-y-scroll relative ">
            <button
            ...
            <div className="pt-4 flex items-center justify-center">
              <form onSubmit={handleSubmit} className={"w-full"}>
                ...
                />
                <button
                  className="flex items-center justify-center w-full gap-2 p-2 border rounded-md text-white bg-green-500 hover:bg-green-600"
                  type="submit"
                >
                  Apply
                </button>
