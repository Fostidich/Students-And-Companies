...
function StudentNotification({ notification, onDelete }) {
  ...
  const getAdvertisement = async () => {
    try {
      ...
      const response = await fetch(
        `${API_SERVER_URL}/api/recommendation/advertisements/${notification.advertisementId}`,
        {
          headers: {
  ...
  const getProfileCompanyDetails = async (companyId) => {
    try {
      const authData = JSON.parse(Cookies.get("authData"));
      const response = await fetch(
        `${API_SERVER_URL}/api/profile/company/${companyId}`,
        {
          headers: {
            Authorization: `Bearer ${authData.token}`,
          },
  ...
  const deleteNotification = async () => {
    try {
      const authData = JSON.parse(Cookies.get("authData"));
      const response = await fetch(
        `${API_SERVER_URL}/api/notification/delete/${notification.studentNotificationId}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${authData.token}`,
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch(
      `${API_SERVER_URL}/api/enrollment/applications/${notification.advertisementId}`,
      {
        method: "POST",
        headers: {
    ...
    <div>
      <div className="bg-white p-4 rounded-lg shadow-xl mb-4 border">
        {notification.type === "INVITED" ? (
          <p className="text-sm font-medium flex justify-around rounded-md bg-blue-500 text-white">
            You have been invited from the company to apply for this
            advertisement
          </p>
        ) : (
          <div></div>
        ...
        {notification.type === "RECOMMENDED" ? (
          <p className="text-sm font-medium flex justify-around rounded-md bg-purple-500 text-white">
            Let me recommend you this internship, you fit well
          </p>
        ) : (
          <div></div>
        )}
        {notification.type === "ACCEPTED" ? (
          <p className="text-sm font-medium flex justify-around rounded-md bg-green-500 text-white">
            You have been accepted for this advertisement
          </p>
        ) : (
          <div></div>
        )}
        {notification.type === "REJECTED" ? (
          <p className="text-sm font-medium flex justify-around rounded-md bg-red-500 text-white">
            You have been rejected for this advertisement
          </p>
        ) : (
          <div></div>
        )}
        <div className="space-y-2">
          <h1 className="text-sm">
            <strong>Company:</strong> {profileCompany.username}
          </h1>
          ...
          <button
            ...
            className="text-white float-right rounded-md bg-red-500 hover:bg-red-600 px-4 py-2"
          >
            Delete
          </button>
        </div>
            ...
            <div className="border p-4 rounded-md space-y-2">
              ...
              <div className="mt-2 border-t pt-2">
                <p className="text-xs text-gray-600">
                  <strong>ADV ID:</strong> {advertisement.advertisementId}
                </p>
                <p className="text-xs text-gray-600">
                  <strong>Company ID:</strong> {advertisement.companyId}
                </p>
                <p className="text-xs text-gray-600">
                  <strong>Duration:</strong> {advertisement.duration}
                </p>
                <p className="text-xs text-gray-600">
                  <strong>Total spots:</strong> {advertisement.spots}
                </p>
                <p className="text-xs text-gray-600">
                  <strong>Available spots:</strong> {advertisement.available}
                </p>
                <p className="text-xs text-gray-600">
                  <strong>Status:</strong>{" "}
                  {advertisement.open ? "Open" : "Closed"}
                </p>
                <p className="text-xs text-gray-600">
            ...
            notification.type === "REJECTED" ? (
              ...
              <div className="items-center justify-center">
                <form onSubmit={handleSubmit}>
                  ...
                  />
                  <button
                    className="gap-2 p-2 border rounded-md text-white bg-green-500 hover:bg-green-600"
                    type="submit"
                  >
                    Apply
                  </button>
