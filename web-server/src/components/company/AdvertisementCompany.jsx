...
function AdvertisementCompany({ advertisement }) {
  ...
  const getInternshipList = async () => {
    const response = await fetch(
      `${API_SERVER_URL}/api/internship/advertisements/${advertisement.advertisementId}`,
      {
        headers: {
  ...
  const getRecommendedStudents = async () => {
    try {
      const response = await fetch(
        `${API_SERVER_URL}/api/recommendation/candidates/advertisements/${advertisement.advertisementId}`,
        {
          headers: {
            Authorization: `Bearer ${authData.token}`,
          },
  ...
  const activeInternships = internshipList.filter((internship) =>
    isInternshipActive(internship.endDate),
  );
  const pastInternships = internshipList.filter(
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center pt-40 pb-40">
          <div className="bg-white p-6 rounded-lg w-2/3 max-h-[90vh] overflow-y-scroll relative pb-16">
            ...
            <div className="border p-4 rounded-md space-y-2">
              ...
              <p className="text-sm">
                <strong>Questionnaire:</strong> {advertisement.questionnaire}
              </p>
