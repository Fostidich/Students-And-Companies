...
function ApplicationsNotifications() {
        ...
        for (const ad of advertisementData) {
          try {
            const appResponse = await fetch(
              `${API_SERVER_URL}/api/enrollment/applications/${ad.advertisementId}`,
              {
                headers: {
                  Authorization: `Bearer ${authData.token}`,
                },
            ...
            if (!appResponse.ok) {
              ...
              console.error(
                `Error fetching applications for ad ${ad.advertisementId}:`,
                message,
              );
              continue;
            }
  ...
  const handleNotification = (applicationId) => {
    setPendingApplications((prev) =>
      prev.filter((application) => application.applicationId !== applicationId),
    );
  };
